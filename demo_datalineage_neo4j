...
Detalhe
System
Sistemas de negócio ou tecnologia (id, name)
Process
Processos ETL, batch jobs, integrações (id, name)
Report
Relatórios gerados pelos processos (id, name)
User
Usuários que acionam processos (id, name)
Dataset
Conjuntos de dados consumidos ou produzidos (id, name)
Database
Bases de dados onde datasets são armazenados (id, name)
Log
Registro de auditoria de execução (id, name)

Relacionamentos:
Relacionamento
Semântica
READS
System lê um Dataset (dependência de consumo)
WRITES
System escreve em um Dataset (produção de dados)
S_INPUT_TO_P
System fornece input para um Process
S_OUTPUT_TO_P
System produz output para um Process
P_INPUT_TO_S
Process alimenta um System
P_OUTPUT_TO_S
Process alimenta um System (resultado)
U_INPUT_TO_P
User aciona um Process (rastreabilidade)
P_OUTPUT_TO_R
Process gera um Report
LOGS_TO
Process registra execução em Log (auditoria)
STORED_IN
Dataset está armazenado em Database
...
Identifica todos os sistemas e relatórios afetados por uma falha — em segundos
2
Data Lineage Ponta a Ponta
Rastreia o caminho completo de um dado desde o usuário até o relatório final
3
Governança e Auditoria de Processos
Detecta processos operando sem log — gaps de conformidade invisíveis em ferramentas tradicionais
4
MDM e Detecção de Inconsistências
Identifica sistemas que leem e escrevem no mesmo dataset — causa raiz de conflitos de MDM
5
Mapa para Modernização e Migração
Calcula a sequência ótima de migração com base na topologia de dependências
...
Arquivo
Descrição
Neo4j_PreVenda_DataLineage.docx
Documento completo de pré-venda com modelo de dados, casos de uso, scripts comentados, queries de demo e comparativo Neo4j vs ferramentas tradicionais
...
Capacidade
SQL / Ferramentas Tradicionais
Neo4j
Análise de impacto em cascata
JOINs múltiplos, horas de análise manual
1 query — resultado em segundos
Data Lineage ponta a ponta
Documentação manual em wikis e planilhas
Caminho navegável e sempre atualizado
Gaps de auditoria
Relatórios batch, visão defasada
Query em tempo real sobre todos os processos
MDM — conflitos de dados
Detectado apenas após incidente
Identificação preventiva por padrão de grafo
Planejamento de migração
Meses de workshops e entrevistas
Topologia calculada automaticamente
Rastreabilidade por usuário
Logs dispersos em sistemas isolados
Caminho completo User → Process → Report em 1 query
